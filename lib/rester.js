...
function getGrammarByName(name) {
    var grammars = atom.grammars.getGrammars();
    for (var i = 0, u = grammars.length; i < u; ++i) {
...
function writeResponse(response, editor) {
    return new Promise(function (resolve, reject) {
        ...
        response.on("end", function () {
            ...
            editor.setGrammar(getGrammarByName(
                atom.config.get("rester.responseGrammar", "HTTP Message")));
            resolve();
        });
    });
}
